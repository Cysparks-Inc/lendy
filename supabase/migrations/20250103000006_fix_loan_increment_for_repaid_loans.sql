-- Migration: Fix loan increment functions to use 'repaid' status instead of 'completed'
-- This ensures that members with repaid loans can get new loans with correct increment levels

-- Update get_next_loan_increment function to use 'repaid' status
...
-- Update validate_loan_increment function to use 'repaid' status
...
GRANT EXECUTE ON FUNCTION public.get_next_loan_increment(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.validate_loan_increment(UUID, DECIMAL, INTEGER, TEXT) TO authenticated;
