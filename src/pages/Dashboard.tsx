...
// --- Type Definitions ---
interface DashboardStats { total_customers: number; total_loans: number; total_disbursed: number; total_repaid: number; outstanding_balance: number; active_loans: number; pending_loans: number; defaulted_loans: number; repaid_loans: number; }
interface RecentLoan { id: string; principal_amount: number; status: 'active' | 'repaid' | 'defaulted' | 'pending'; member_name: string; member_id: string; }
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Both RPC calls are now securely filtered based on the user's role.
        const [statsRes, loansRes] = await Promise.all([
          supabase.rpc('get_dashboard_stats_for_user', { requesting_user_id: user.id }),
          supabase.rpc('get_recent_loans_for_user', { requesting_user_id: user.id })
        ]);
  ...
  return (
    ...
    <div className="space-y-6 p-2 sm:p-4 md:p-6">
      ...
      {userRole === 'branch_manager' && <AdminDashboard stats={stats} recentLoans={recentLoans} scope={`Branch: ${profile?.branch_name || 'Your Branch'}`} />}
...
// A single, robust component for both Super Admins and Branch Managers
const AdminDashboard: React.FC<{ stats: DashboardStats; recentLoans: RecentLoan[]; scope: string }> = ({ stats, recentLoans, scope }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard title={`${scope} Disbursed`} value={formatCurrency(stats.total_disbursed)} icon={DollarSign} />
                <StatCard title={`${scope} Outstanding`} value={formatCurrency(stats.outstanding_balance)} icon={TrendingUp} />
                <StatCard title="Total Customers" value={stats.total_customers} icon={Users} />
                <StatCard title={`${scope} Loans`} value={stats.total_loans} icon={CreditCard} subtitle={`${stats.active_loans} active`} />
            </div>
            <div className="grid gap-6 grid-cols-1 lg:grid-cols-3">
                <Card className="lg:col-span-2">
                    <CardHeader><CardTitle>Recent Loans</CardTitle><CardDescription>The last 5 loans created in your scope.</CardDescription></CardHeader>
                    <CardContent>
                        <DataTable columns={recentLoanColumns} data={recentLoans} emptyStateMessage="No recent loans found."/>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader><CardTitle>Portfolio Health</CardTitle><CardDescription>Key performance indicators for your scope.</CardDescription></CardHeader>
...
const LoanOfficerView: React.FC<{ stats: DashboardStats; recentLoans: RecentLoan[] }> = ({ stats, recentLoans }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader><CardTitle>My Recent Activity</CardTitle><CardDescription>Your 5 most recently created loans.</CardDescription></CardHeader>
                <CardContent>
